...
export async function UpdatedMangaGrid({ filter, count }: UpdatedMangaGridProps) {
  const howMany = await getCookieForRefetching();
  const { error, data, offset, total } = await fetchUpdatedMangasToGrid(filter, count, 0, howMany);
  ...
  async function loadMoreMangas(offset: number): Promise<MangaGridResponse> {
    'use server';
    const { error, data, offset: newOffset, total } = await fetchUpdatedMangasToGrid(filter, count, offset);
